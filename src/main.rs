...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Util(util_command) => match util_command {
            UtilCommand::Completions { directory } => {
                ...
                for shell in Shell::value_variants() {
                    ...
                    match std::fs::write(path.join(shell.file_name("tapoctl")), completions) {
                        Ok(_) => println!("Successfully created completions for {shell}"),
                        Err(err) => {
                            println!("Error whilst writing completions file for {shell}: {err}")
                        }
                    }
                }
            }
        ...
        Commands::Client(client_command) => {
            ...
            let mut spinner =
                (!json).then(|| Spinner::new(spinners::Dots, "Preparing client...", None));
            ...
            match client_command {
                ClientCommand::Devices => {
                    let devices = client
                        .devices(Empty {})
                        .await
                        .map_tonic_err(&mut spinner, json)
                        .into_inner();
                ...
                ClientCommand::Info { device } => {
                    if json {
                        let json = client
                            .info_json(DeviceRequest { device })
                            .await
                            .map_tonic_err(&mut spinner, json);
                        let value: HashMap<String, Value> =
                            serde_json::from_slice(json.into_inner().data.as_slice()).unwrap();
                        println!("{}", json!(value));
                    } else {
                        let info = client
                            .info(DeviceRequest { device })
                            .await
                            .map_tonic_err(&mut spinner, json)
                            .into_inner();
                ...
                ClientCommand::Events { types } => {
                    let request = EventRequest {
                        types: types.into_iter().map(i32::from).collect(),
                    };
...
async fn get_client(
    ...
    let port = std::env::var("TAPO_PORT")
        .map(|p| u16::from_str(p.as_str()).unwrap_or(port))
        .unwrap_or(port);
