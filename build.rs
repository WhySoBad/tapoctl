...
fn custom_validator(fn_name: &str) -> String {
    "#[validate(custom(function = \"".to_owned()
        + CUSTOM_VALIDATOR_LOCATION
        + "::"
        + fn_name
        + "\"))]"
}
...
fn build_tonic() -> Result<(), Box<dyn std::error::Error>> {
    tonic_build::configure()
        ...
        .field_attribute(
            "tapo.SetRequest.brightness",
            custom_validator("validate_brightness"),
        )
        .field_attribute(
            "tapo.SetRequest.temperature",
            custom_validator("validate_temperature"),
        )
        .field_attribute(
            "tapo.SetRequest.hue_saturation",
            custom_validator("validate_hue_saturation"),
        )
        .field_attribute("tapo.SetRequest.hue_saturation.hue", VALIDATOR_REQUIRED)
        .field_attribute(
            "tapo.SetRequest.hue_saturation.saturation",
            VALIDATOR_REQUIRED,
        )
